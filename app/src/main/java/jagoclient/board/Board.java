...
//2015/07/24 //1Ad9 2015/07/21 additional to 1Ad6(OutOfMemory)     //~1Ad9I~
//v1B7 2014/08/11 display komi for "local count"                   //~v1B7I~
...
//import java.awt.Canvas;                                          //~1116R~
//import java.awt.Color;                                           //~1116R~
//import java.awt.Dimension;                                       //~1116R~
//import java.awt.Font;                                            //~1116R~
//import java.awt.FontMetrics;                                     //~1116R~//~1117R~
//import java.awt.Frame;                                           //~1116R~
//import java.awt.Graphics;                                        //~1116R~
//import java.awt.Image;                                           //~1116R~
//import java.awt.event.KeyEvent;                                  //~1116R~
//import java.awt.event.KeyListener;                               //~1116R~
//import java.awt.event.MouseEvent;                                //~1116R~
//import java.awt.event.MouseListener;                             //~1116R~
//import java.awt.event.MouseMotionListener;                       //~1116R~
//import java.awt.image.ColorModel;                                //~1116R~
//import java.awt.image.MemoryImageSource;                         //~1116R~
...
import com.Ajagoc.awt.ColorModel;
...
import com.Ajagoc.awt.MouseEvent;
import com.Ajagoc.awt.MouseListener;
import com.Ajagoc.awt.MouseMotionListener;
...
{	int O,W,D,S,OT,OTU,OP; // pixel coordinates
  ...
  public                                                           //~1B33I~
    ...
    private String strKomi;                                        //~v1B7I~
    public  String strResult;                                      //~v1B7I~
    public  int countRule;                                         //~v1B7I~
    public  static final int RULE_JAPANESE=1;                       //~v1B7I~
    public  static final int RULE_CHINESE=2;                       //~v1B7I~
    public  int resignColor;                                       //~v1B7R~
	// ******************** initialize board *******************

	public Board (int size, BoardInterface gf)                     //~1420R~
	{	S=size; D=16; W=S*D;
        if (Dump.Y) Dump.println("New Board gf="+gf.toString());   //~1Ad9R~
		Empty=null;
		EmptyShadow=null;
		showlast=true;
		GF=gf;
...
//Ajust for Gravity=Center                                         //~@@@1R~
     ...
     {                                                             //~1B0gI~
      {//Ajust for Gravity=Center @@@@                             //~1427I~
        OP=0;                                            //@@@@ no margin//~1427M~
        int coordspace=(c?((ulc?1:0)+(lrc?1:0)):0);                //~1427R~
        W=Dim.height;                                              //~1427M~
        D=W/(S+coordspace);                                        //~1427R~
        int margin;
        margin=(W-D*(S+coordspace))/2;                             //~1502R~
        if (margin<D/2 && coordspace!=2)  //no space except coordinates//~1502R~
        {                                                          //~1501R~
            D=W/(S+coordspace+1);       //add "D" for margin to touch//~1502R~
            margin=(W-D*(S+coordspace))/2;                         //~1502R~
        }                                                          //~1501R~
        O=margin;                                                  //~1502R~
        if (Dump.Y) Dump.println("coord W="+W+",margin="+margin+",O="+O+",D="+D);//~1Ad9R~
      }//@@@@                                                      //~1427I~
		if (c)
		{	if (lrc) OT=D;
			else OT=0;
...
//  	W+=OTU+OT;      //@@@@                                     //~1427R~
        ...
        if (Dump.Y) Dump.println("Board:makeimages W="+W);         //~1Ad9R~
		// create image and paint empty board
        Image empty=Empty;	//recycle bitmap @@@@                  //~1421I~
        Image emptyshadow=EmptyShadow;     //@@@@                  //~1421I~
		synchronized (this)
        ...
        empty=null;                                                //~1Ad9I~
        recycle(emptyshadow);           //@@@@                     //~1422R~
        emptyshadow=null;                                          //~1Ad9I~
		emptypaint();
		...
		swEmpty=false;		//@@@@                                 //~1422I~
		repaint();
        if (Dump.Y) Dump.println("Board:makeimages return");         //~1506R~//~1B0gR~
     }                                                             //~1B0gI~
     catch(OutOfMemoryError me)                                    //~1B0gI~
     {                                                             //~1B0gI~
        Dump.println(me,"Board:makeimages");                       //~1B0gI~
        AjagoView.memoryShortage("Board:makeimage");                //~1B0gI~
     }                                                             //~1B0gI~
	}
	...
	{	Dimension d=getSize();
        if (Dump.Y) Dump.println("paint@ start Dim=("+Dim.width+","+Dim.height+"),d=("+d.width+","+d.height+")");//~1Ad9R~
        if (Dump.Y) Dump.println("paint@ Graphics="+g.toString()+",this board="+this.toString());//~1Ad9R~
        ...
        if (Dump.Y) Dump.println("paint W="+W+",dim w="+d.width+",h="+d.height);//~1Ad9R~
		if (d.width>W)
			g.fillRect(W,0,d.width-W,W);
		if (d.height>W)
			g.fillRect(0,W,d.width,d.height-W);
        if (Dump.Y) Dump.println("paint@ end");                    //~1Ad9R~
	}
	...
	*/
	...
	{	if (EmptyPaint.haveImage(w,w,
		...
		{	gr.drawImage(EmptyPaint.StaticImage,O+OTU-OP,O+OTU-OP,this);
        	if (Dump.Y) Dump.println("Board:trywood before recycle StaticImage="+EmptyPaint.StaticImage.toString());//~1Ad9I~
        	if (Dump.Y) Dump.println("Board:trywood before recycle StaticImage bitmap="+EmptyPaint.StaticImage.bitmap.toString());//~1Ad9I~
            ...
            {                                                      //~@@@1I~
	        	...
	        	if (Dump.Y) Dump.println("Board:trywood before recycle StaticShadowImage bitmap="+EmptyPaint.StaticShadowImage.bitmap.toString());//~1Ad9I~
	...
	{	int col=GF.boardColor().getRGB();
        ...
        if (Dump.Y) Dump.println("StonesPrint started");           //~1506R~
		...
		{	int d=D+2;
        	...
        	Image blackstone=BlackStone;    //@@@@                 //~1421I~
        	Image whitestone=WhiteStone;    //@@@@                 //~1421I~
			...
			WhiteStone=createImage(
				new MemoryImageSource(d,d,ColorModel.getRGBdefault(),
							pw,0,d));
	        recycle(blackstone);          //@@@@                   //~1422R~
	        recycle(whitestone);          //@@@@                   //~1422R~
		}
        if (Dump.Y) Dump.println("StonesPrint End");               //~1506R~
	}
...
//*from canvas,draw cursor when showTarget is On*****************************//~1B33I~
	public void drawTarget (android.graphics.Canvas Pcanvas,Paint Ppaint,int Pposx,int Pposy)//~1B33R~
	{                                                              //~1B33I~
//        if (GF.bwColor()) Ppaint.setColor((Color.white).getRGB());//~1B33R~
//        else Ppaint.setColor((Color.gray.brighter()).getRGB());  //~1B33R~
        Ppaint.setColor((Color.white).getRGB()); //gray.brighter is not easy to find//~1B33I~
        Rect rect=new Rect(Pposx-D/4,Pposy-D/4,Pposx+D/4,Pposy+D/4);//~1B33R~
		Pcanvas.drawRect(rect,Ppaint);                             //~1B33R~
	}                                                              //~1B33I~
    ...
    public synchronized void keyPressed (KeyEvent e)
  	{	if (e.isActionKey())
  		...
  		{                                                          //~1116R~
	    	if (Dump.Y) Dump.println("Board.keyPressed char="+Integer.toHexString((int)(e.getKeyChar())));//~1Ad9R~
			switch (e.getKeyChar())                                //~1116I~
  			{	case '*' : varmain(); break;
...
//      		case KeyEvent.VK_BACK_SPACE :               //@@@@chked as isActionKey//~1428R~
//				case KeyEvent.VK_DELETE : undo(); break;    //@@@@by isActionKey()==true//~1428R~
  			}
  		}
  	}
	...
	{	Node n=Pos.node();
        ...
        if (Dump.Y) Dump.println("Board:showinformation icon button update start");//~1Ad9R~
        ...
        if (Dump.Y) Dump.println("Board:showinformation icon button update End");//~1Ad9R~
		int i,j;
		...
		{	a=(Action)la.content();
			...
			{	sc=(String)a.arguments().content();
            	if (Dump.Y) Dump.println("Board:showinformation a.type='C':"+sc);//~v1B7I~
			}
		...
		{	if (GF.lastNumber() || (Range>=0 && P.number(i,j)>Range))
			...
			{	if (GF.bwColor())
				{	if (P.color(i,j)>0) g.setColor(Color.white);
    			    ...
    			    g.drawLine(xi+D/2-D/6,xj+D/2,xi+D/2+D/6,xj+D/2);//~1B31I~
    		  	    g.drawLine(xi+D/2,xj+D/2-D/6,xi+D/2,xj+D/2+D/6);//~1B31I~
				}
//  			else g.setColor(Color.red);                        //~1B31R~
    			else                                               //~1B31I~
				{                                                  //~1B31I~
    				g.setColor(Color.red);                         //~1B31I~
	    			g.drawLine(xi+D/2-D/4,xj+D/2,xi+D/2+D/4,xj+D/2,3/*linewidth*/);//~1B31I~
    				g.drawLine(xi+D/2,xj+D/2-D/4,xi+D/2,xj+D/2+D/4,3);//~1B31I~
				}                                                  //~1B31I~
//  			g.drawLine(xi+D/2-D/6,xj+D/2,xi+D/2+D/6,xj+D/2);   //~1B31R~
//  			g.drawLine(xi+D/2,xj+D/2-D/6,xi+D/2,xj+D/2+D/6);   //~1B31R~
			}
		}
	...
	{	clearmarks();
		...
		showinformation();
		copy();
        double diffc=sw+tw+-(sb+tb);   //chinese                   //~v1B7R~
        double diffj=Pb+tw+-(Pw+tb);   //japanese                  //~v1B7R~
        String result=getResult(diffc,diffj);                      //~v1B7I~
		return GF.resourceString("Chinese_count_")+"\n"+
			GF.resourceString("Black__")+(sb+tb)+
			GF.resourceString("__White__")+(sw+tw)+"\n"+
			GF.resourceString("Japanese_count_")+"\n"+
			GF.resourceString("Black__")+(Pw+tb)+
//  		GF.resourceString("__White__")+(Pb+tw);                //~v1B7R~
    		GF.resourceString("__White__")+(Pb+tw)                 //~v1B7I~
            +result;                                               //~v1B7R~
	}
...
//@@@@  EmptyShadow=null;           //make image set new           //~1420R~
		setfonts();
        makeimages();               //executed at paint()          //~1420R~
...
//**************************************************************** //~v1B7I~
//*from GTPGoFrame(human resign) and GTPConnection(computer resign)//~v1B7I~
//**************************************************************** //~v1B7I~
	public void resigned()                                         //~v1B7I~
	{                                                              //~v1B7I~
    	resignColor=MainColor;                                     //~v1B7I~
	}                                                              //~v1B7I~
//**************************************************************** //~v1B7I~
//*from GTPConnection                                              //~v1B7I~
//**************************************************************** //~v1B7I~
	public void resetresigned()                                    //~v1B7I~
	{                                                              //~v1B7I~
    	resignColor=0;                                           //~v1B7I~
	    strResult=null;                                            //~v1B7I~
	}                                                              //~v1B7I~
//**************************************************************** //~v1B7I~
	private String getResult(double Pdiffc,double Pdiffj)          //~v1B7R~
    {                                                              //~v1B7I~
        String resultc,resultj,result="";                          //~v1B7I~
        double komi=0.0;                                           //~v1B7I~
        if (resignColor!=0)                                        //~v1B7I~
        {                                                          //~v1B7I~
        	if (resignColor>0)	//black resigned                   //~v1B7R~
                result="\n"+GF.resourceString("WhiteHead")+"+ ("+GF.resourceString("BlackHead")+" "+GF.resourceString("Resigned")+")";//~v1B7R~
            else                                                   //~v1B7I~
                result="\n"+GF.resourceString("BlackHead")+"+ ("+GF.resourceString("WhiteHead")+" "+GF.resourceString("Resigned")+")";//~v1B7R~
	        strResult=result;                                      //~v1B7I~
        }                                                          //~v1B7I~
        else                                                       //~v1B7I~
      	if (strKomi!=null) //GTP                                   //~v1B7I~
      	{                                                          //~v1B7I~
            komi=AjagoUtils.str2double(strKomi,0);                 //~v1B7I~
            double diffc=Pdiffc+komi;	                           //~v1B7I~
            double diffj=Pdiffj+komi;                              //~v1B7I~
            if (diffc>0)                                           //~v1B7R~
                resultc=GF.resourceString("WhiteHead")+"+"+Double.toString(diffc);//~v1B7R~
            else                                                   //~v1B7I~
            if (diffc<0)                                           //~v1B7R~
                resultc=GF.resourceString("BlackHead")+"+"+Double.toString(-diffc);//~v1B7R~
            else                                                   //~v1B7I~
                resultc=GF.resourceString("Even");                 //~v1B7I~
            if (diffj>0)                                           //~v1B7R~
                resultj=GF.resourceString("WhiteHead")+"+"+Double.toString(diffj);//~v1B7R~
            else                                                   //~v1B7I~
            if (diffj<0)                                           //~v1B7R~
                resultj=GF.resourceString("BlackHead")+"+"+Double.toString(-diffj);//~v1B7R~
            else                                                   //~v1B7I~
                resultj=GF.resourceString("Even");                 //~v1B7I~
            if (countRule==RULE_JAPANESE)                          //~v1B7I~
                strResult=resultj+"("+GF.resourceString("Japanese_count_")+")";//~v1B7I~
            else                                                   //~v1B7I~
            if (countRule==RULE_CHINESE)                           //~v1B7I~
                strResult=resultc+"("+GF.resourceString("Chinese_count_")+")";//~v1B7I~
            else                                                   //~v1B7I~
                strResult=resultc+"("+GF.resourceString("Chinese_count_")+")"+"  "+//~v1B7I~
                          resultj+"("+GF.resourceString("Japanese_count_")+")";//~v1B7I~
      	}                                                          //~v1B7I~
        result=GF.resourceString("Result")+":"+strResult;          //~v1B7I~
      	if (strKomi!=null) //GTP                                   //~v1B7I~
        	result="\n"+GF.resourceString("Komi")+":"+(strKomi!=null ? strKomi : "0")+"\n"+result;//~v1B7I~
        return result;                                             //~v1B7I~
    }                                                              //~v1B7I~
    //*************************************************************************//+1Ag9I~
    ...
    {                                                              //+1Ag9I~
		...
		if (EmptyShadow!=null)                                           //+1Ag9I~
        {                                                          //+1Ag9I~
        	if (Dump.Y) Dump.println("Board:onDestroy EmptyShadow="+EmptyShadow.toString());//+1Ag9I~
        	EmptyShadow.recycle(true);                             //+1Ag9I~
            EmptyShadow=null;                                      //+1Ag9I~
        }                                                          //+1Ag9I~
