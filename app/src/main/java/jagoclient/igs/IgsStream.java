...
{	String Line;
	...
	*/
	...
	{	CF=cf;
		...
		L=0;
		Number=0;
		if (Dump.Y) Dump.println("--> IgsStream opened");          //~1506R~
	...
	*/
	...
	{	try
		{	InputStream ina;
            ...
            if (Dump.Y) Dump.println("stream encoding="+encoding); //+6924R~
	...
	{	while (true)
		{                                                          //~1218R~
			if (Ajagoc.isTerminating()) throw new IOException();  //@@@@ break infinite loop//~1218R~
			int c=In.read();                                       //~1218I~
//@@@@                                                             //~1219I~
          	for (int ii=0;;)  //detect shutdown                    //~1219I~
          	{                                                      //~1219I~
//      		if (Dump.Y) Dump.println("IgsStream read:"+Integer.toString((int)c,16));//~1218R~//~1511R~
            	if (c!=0xfffd)	//unicode subchar                  //~1219R~
                	break;                                         //~1219R~
				if (available())	//not avai after socket shutdown//~1219R~
            		break;                                         //~1219R~
				if (Dump.Y) Dump.println("IgsStream read:fffd ctr="+ii);//~1511R~
          		if (ii++>=100)                                     //~1219R~
					throw new IOException();                       //~1219R~
				c=In.read();                                       //~1219I~
          }                                                        //~1219R~
//@@@@                                                             //~1219I~
	...
	*/
	...
	{	StringParser sp;
		...
		while (true)
		{	if (L>=linesize || b=='\n')
			{	if (L>=linesize) if (Dump.Y) Dump.println("IGS : Buffer overflow");//~1506R~
				break;
			}
			C[L++]=b;
			b=read();
		...
		{	Number=Integer.parseInt(new String(C,0,i),10);
          if (i<L)  //@@@@ for safety of OutOfBound exception      //~1311I~
	...
	{	while (true)
		{	readlineprim();
			...
			{	if (dis.once())
				{	unchain(dis);
					dis.finished();
					if (Dump.Y) Dump.println("Distributor "+dis.number()+" finished");//~1506R~
				}
				break;
			}
		}
		if (Dump.Y) Dump.println("sendall() for "+dis.number()+" finished");//~1506R~
		dis.allsended();
	}

	/**
	Same as above, but the distrubutor get the input with
	the String prepended.
	*/
	void sendall (String s, Distributor dis) throws IOException
	{	while (true)
		{	readlineprim();
			if (Dump.Y) Dump.println("IGS: "+Command);             //~1506R~
	...
	<P>
	The protocol is not very logic, nor is the structure of this
	method. It has cases for several distributors. Probably, this
	code should be a static method of the distributor.
	...
	*/
	...
	{	boolean full;
		...
		{	full=false;
			...
			while (true)
			{	if (L>=linesize)
				{	if (Dump.Y) Dump.println("IGS : Buffer overflow");//~1506R~
			...
			if (Dump.Y) Dump.println("IGS sent: "+Line);           //~1506R~
			Number=0;
			Command="";
			if (full)
			{	int i;
				...
				L=0;
				loop1: while (true)
				{	if (Dump.Y) Dump.println("loop1 with "+Number+" "+Command);//~1506R~
					if (Number==21 && 
					...
					{	sp=new StringParser(Command);
						...
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Sending comment to game "+G);//~1506R~
							dis.send(Command);
					...
					{	sp=new StringParser(Command);
						...
						Distributor dis=findDistributor(15,G);
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Sending kibitz to game "+G);//~1506R~
							dis.send(Command);
							sendall("Kibitz-> ",dis);
							continue loop1;
						}
					...
					{	sp=new StringParser(Command);
						...
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Got "+Command);//~1506R~
							dis.send(Command);
							sendall(dis);
							continue loop1;
						}
					...
					{	Distributor dis=findDistributor(9);
						...
						{	if (Dump.Y) Dump.println("Sending information");//~1506R~
							dis.send(Command);
							sendall(dis);
							if (Dump.Y) Dump.println("End of information");//~1506R~
							continue loop1;
						}
					}
					...
					{	sp=new StringParser(Command);
						...
						Distributor dis=findDistributor(15,G);
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Sending to game "+G);//~1506R~
							dis.send(Command);
							sendall(dis);
							continue loop1;
						}
						...
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Game "+G+" started");//~1506R~
							dis.game(G);
							dis.send(Command);
							sendall(dis);
							continue loop1;
					...
					{	sp=new StringParser(Command);
						...
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Sending to channel "+G);//~1506R~
					...
					{	Distributor dis=findDistributor(32);
						if (dis!=null)
						{	if (Dump.Y) Dump.println("Sending to channel "+dis.game());//~1506R~
							dis.send(Command);
							continue outerloop;
						}
						continue outerloop;
					...
					{	Distributor dis=findDistributor(Number);
						...
						else if (Dump.Y) Dump.println("Distributor "+Number+" not found");//~1506R~
					}
					break;
				}
				L=0;
	...
	*/
	...
	{	if (s.startsWith("observe") || s.startsWith("moves")
		...
		Out.println(s);
		Out.flush();
		if (Dump.Y) Dump.println("Sending: "+s);                   //~1506R~
	}
