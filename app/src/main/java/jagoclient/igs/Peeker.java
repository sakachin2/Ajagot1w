...
{	IgsGoFrame GF;
	...
	{	StringParser p;
		int nu,i,j;
		if (Dump.Y) Dump.println("Peeked: "+s);                    //+1506R~
		...
		{	int hn;
			p.skipblanks();
			hn=p.parseint();
			if (Dump.Y) Dump.println("Peeker read: Handicap "+hn); //+1506R~
			GF.handicap(hn);
			return;
		}
		i=m.charAt(0)-'A';
		...
		if (i<0 || j<0) return;
		if (Dump.Y) Dump.println("Peeker read: "+c+" "+i+","+j);   //+1506R~
		if (c.equals("W")) GF.white(i,BS-1-j);
		else GF.black(i,BS-1-j);
	}

	void finished ()
	{	if (Dump.Y) Dump.println("Peeker is finished");            //+1506R~
		GF.active(true);
	}
