...
{	GoObserver GO;
	...
	{	GO=go;
		start();
        if (Dump.Y) Dump.println("ObserverCloser thread start");   //+6924R~
	}
	public void run ()
	{	try
		{	sleep(10000);
        ...
        if (Dump.Y) Dump.println("ObserverCloser thread after sleep 10sec");//+6924R~
		GO.finishremove();
	}
}
...
{	IgsGoFrame GF;
	...
	{	if (Closed || !Observing) return;
		StringParser p;
		int nu,i,j;
		if (Dump.Y) Dump.println("Observed("+N+"): "+s);           //~1506R~
		...
		{	if (Dump.Y) Dump.println("Oberver denies number "+nu+" expecting "+Expected);//~1506R~
			Out.println("moves "+N);
			Got=nu;
			return;
		}
		...
		if (i<0 || j<0) return;
		if (Dump.Y) Dump.println("GoObserver interpreted: "+c+" "+i+","+j);//~1506R~
		if (Dump.Y) Dump.println("GoObserver white/black GF="+GF.toString());//~1506R~
		if (c.equals("W")) GF.white(i,BS-1-j);
		else GF.black(i,BS-1-j);
	}

	public void finished ()
	{	if (Dump.Y) Dump.println("GoObserver("+N+") is finished"); //~1506R~
	}

	public void remove ()
	{	if (Dump.Y) Dump.println("GoObserver("+N+") has ended, unobserving");//~1506R~
		Closed=true;
	...
	{	if (i>=8) i++;
		...
		if (Dump.Y) Dump.println("***> Player sends "+new String(c)+(BS-j));//~1506R~
	}

	public void pass ()
	{	Out.println("pass");
		if (Dump.Y) Dump.println("***> Player passes");            //~1506R~
	}
