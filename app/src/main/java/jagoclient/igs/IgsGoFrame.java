...
/**
This is a ConnectedGoFrame, which is used to display boards
...
public class IgsGoFrame extends ConnectedGoFrame 
    implements TimedBoard, OutputListener, KeyListener                                                          //~1116I~
{	Distributor Dis; // Distributor for this frame
	...
	{	super(s,19,Global.resourceString("Remove_groups"),
		...
		Options.add(Terminal=new CheckboxMenuItemAction(this,
			Global.resourceString("Display_Terminal_Output")));
		// Fix for Linux Java 1.5:
		try { Terminal.setState(Global.getParameter("getterminal",true)); } catch (Exception e){}
		setterminal();
		Options.add(ShortLines=new CheckboxMenuItemAction(this,
			Global.resourceString("Short_Lines_only")));
		try { ShortLines.setState(Global.getParameter("shortlinesonly",true)); } catch (Exception e) {}
		addKeyListener(this);
	...
	*/
	...
	{	String S;
        ...
        if (Dump.Y) Dump.println("IgsGoFrame settitle1 "+WhiteName+"-"+BlackName);//+@@@1I~
	}
	
	void settitle (String s)
	{	Title=s;
		setTitle(s);
        if (Dump.Y) Dump.println("IgsGoFrame settitle "+s);        //+@@@1I~
	}
