...
{	IgsGoFrame GF;
	...
	*/
	...
	{	if (Dump.Y) Dump.println("Sizer got: "+s);                 //+1506R~
		if (L==1) White=s;
		else if (L==2) Black=s;
		else
		{	while (true)
	...
	*/
	...
	{	if (Dump.Y) Dump.println("Sizer has size "+BS);            //+1506R~
		HaveSize=true;
		GF.doboardsize(BS);
		setinformation();
		Out.println("moves "+N);
	...
	*/
	...
	{	if (!HaveSize) // get board size first of all
		...
		StringParser p;
		int nu,i,j;
		if (Dump.Y) Dump.println("Player("+N+"): "+s);             //+1506R~
		...
		{	if (s.startsWith("Game")) GF.settime(s); // time information
			...
			{	p.skip("Removing @");
				...
				if (i<0 || j<0) return;
				if (Dump.Y) Dump.println("Removing "+i+","+j);     //+1506R~
				GF.remove(i,BS-1-j);
			}
			return;
		}
		...
		{	if (Dump.Y) Dump.println("Player denies number "+nu+" expecting "+Expected);//+1506R~
			Got=nu;
			return;
		}
		else if (Expected>nu)
		...
		if (Dump.Y) Dump.println("Player interpreted: "+c+" "+i+","+j);//+1506R~
		if (c.equals("W")) GF.white(i,BS-1-j);
		else GF.black(i,BS-1-j);
	}

	void remove ()
	{	if (Dump.Y) Dump.println("Player("+N+") has ended");       //+1506R~
	...
	*/
	...
	{	if (i>=8) i++;
		...
		Out.println(s);
		if (Dump.Y) Dump.println("***> Player sends "+s);          //+1506R~
	}
